...
namespace darknet_ros {
    ...
    void YoloObjectDetector::onInit() {
        ...
        nodeHandle_ = getPrivateNodeHandle();
        imageTransport_ = image_transport::ImageTransport(nodeHandle_);
    ...
    bool YoloObjectDetector::readParameters() {
        ...
        nodeHandle_.param("/darknet_ros/image_view/enable_console_output", enableConsoleOutput_, false);
    ...
    void YoloObjectDetector::init() {
        ...
        nodeHandle_.param("/darknet_ros/yolo_model/weight_file/name", weightsModel, std::string(""));
        ...
        // Path to config file.
        nodeHandle_.param("/darknet_ros/yolo_model/config_file/name", configModel, std::string(""));
        ...
        // If config is incopmplete
        if (configModel.empty() || weightsModel.empty()){
            ROS_FATAL("/darknet_ros/yolo_model/weight_file/name or /darknet_ros/yolo_model/config_file/name is invalid.");
            ros::requestShutdown();
        }
        ...
        std::string positionsAndProbabilitiesTopicName;
        int positionsAndProbabilitiesQueueSize;
        bool positionsAndProbabilitiesLatch;
        ...
        nodeHandle_.param("/darknet_ros/publishers/object_detector/topic", objectDetectorTopicName, std::string("found_object"));
        ...
        nodeHandle_.param("/darknet_ros/publishers/bounding_boxes/topic", boundingBoxesTopicName, std::string("bounding_boxes"));
        ...
        nodeHandle_.param("/darknet_ros/publishers/positions_and_probabilities/topic", positionsAndProbabilitiesTopicName, std::string("positions_and_probabilities"));
        nodeHandle_.param("/darknet_ros/publishers/positions_and_probabilities/queue_size", positionsAndProbabilitiesQueueSize, 1);
        nodeHandle_.param("/darknet_ros/publishers/positions_and_probabilities/latch", positionsAndProbabilitiesLatch, false);

        nodeHandle_.param("/darknet_ros/publishers/detection_image/topic", detectionImageTopicName, std::string("detection_image"));
        ...
        nodeHandle_.param("/darknet_ros/actions/camera_reading/topic", checkForObjectsActionName, std::string("check_for_objects"));
    ...
    void YoloObjectDetector::drawBoxes(cv::Mat &inputFrame, std::vector<RosBox_> &rosBoxes, int &numberOfObjects,
        ...
        for (int i = 0; i < numberOfObjects; i++) {
            ...
            cv::putText(inputFrame,
                        objectLabel + " (" + probability.str() + "%)",
                        cv::Point(xmin, ymax+15),
    ...
    void YoloObjectDetector::runYolo(cv::Mat &fullFrame, const std_msgs::Header& header, int id) {
        ...
        if (num > 0  && num <= 100) {
            ...
            positionsAndProbabilities_.header = header;
            positionsAndProbabilitiesPublisher_.publish(positionsAndProbabilities_);
